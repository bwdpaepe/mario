...
public class RenderBatch {
    ...
    public void render() {
        ...
        // use shader
        this.shader.use();
        this.shader.uploadMat4f("uProjection", Window.getScene().camera().getProjectionMatrix());
        this.shader.uploadMat4f("uView", Window.getScene().camera().getViewMatrix());
        int i = 0;
        for(Texture tx: this.textures){
            glActiveTexture(GL_TEXTURE0 + i + 1);
            tx.bind();
            i++;
        }
        this.shader.uploadIntArray("uTextures", texSlots);
